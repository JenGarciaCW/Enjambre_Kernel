...
{
	/*
	 * Solo necesita dirección interna
	 */
    ...
    serv_addr.sin_addr.s_addr = inet_addr(this->host);		//Dirección IP de la máquina
    ...
    listen(this->sockfd,1);		// espera hasta establecer una conección (1 conexión en lista de espera)
...
{
	/*
	 * Necesita  dirección del servidor
	 */
...
void socket_message::init_udp_receiver_socket()
{
	 ...
	 */
	    ...
	    this->serv_addr.sin_addr.s_addr = inet_addr(this->host);				//Dirección IP de la máquina
	    this->serv_addr.sin_port = htons(this->portno);			//Puerto de conexión - host to network short
	    //serv_addr.sin_zero Números complemento

	    if (bind(this->sockfd, (struct sockaddr *) &(this->serv_addr),sizeof(this->serv_addr)) < 0)	 //asigna dirección y puerto al identificador de socket
	    	error("ERROR on binding");	//regresa -1 si no es posible
