...
float AS5048::readang()
{
	...
	this->readreg(0x0FE);
	std::cout<<std::hex<<(int)this->buffer[0]<<"\t"<<std::hex<<(int)this->buffer[1]<<"\t";
	return ((int16_t)this->buffer[1] << 6 | ((int16_t)this->buffer[0] & 0x3F))* 360.0 / 16384.0; //3E
}

char AS5048::diagnostic()
{
	this->buffsize = 1;
	this->buffer = new unsigned char [1];
	this->readreg(0x0FB);
	return (this->buffer[0]) ; //3E
}
