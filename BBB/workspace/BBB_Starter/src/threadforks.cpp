...
void* Func(void * a)
{
	...
	((threadforks *)a)->ready=1;
	    ...
	    default: // parent process, pid now contains the child pid
	        while (-1 == waitpid(((threadforks *)a)->pid, &((threadforks *)a)->status, 0)); // wait for child to complete
	        if (!WIFEXITED(((threadforks *)a)->status) || WEXITSTATUS(((threadforks *)a)->status) != 0)
	        {
	            // handle error
	            std::cerr << "process " << ((threadforks *)a)->cmd[j] << " (pid=" << ((threadforks *)a)->pid << ") failed" << std::endl;
	        }
	        break;
	    }
}
